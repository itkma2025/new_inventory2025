        ...
        <div class="card">
          <div class="card-body p-3">
            ...
            $sql = "SELECT 
                      ...
                      tbsm.kode_set_marwa,
                      tbsm.nama_set_marwa,
                      tbsm.harga_set_marwa,
                      lok.nama_lokasi,
                      lok.no_lantai,
                      lok.nama_area,
                      lok.no_rak
                    FROM tb_produk_set_marwa AS tbsm 
                    LEFT JOIN tb_lokasi_produk AS lok ON (tbsm.id_lokasi = lok.id_lokasi)
                    WHERE id_set_marwa = '$decrypt_id'";
                  ...
                  <div class="row">
                    <div class="col-sm-8">
                      ...
                      $sql_data = "SELECT 
                                  ...
                                  WHERE tpsm.id_set_marwa = '$decrypt_id' ORDER BY tpr.nama_produk ASC";
                      ...
                      ?>
                      <?php } ?>
                      <p>Harga Modal (Rp) : <?php echo number_format($grand_total, 0, '.', '.'); ?></p>
                      <p>Harga Jual (Rp) : <?php echo number_format($data['harga_set_marwa'], 0, '.', '.'); ?></p>
                      <?php  
                        if($data['nama_lokasi'] != "" OR $data['no_lantai'] != "" OR $data['nama_area'] != "" OR $data['no_rak'] != ""){
                          ?>
                    ...
                    <div class="col-sm-4 text-end mb-3">
                      <a href="tambah-isi-produk-set-marwa.php?id-set=<?php echo $encrypt_id ?>" class="btn btn-primary"><i class="bi bi-plus-circle"></i> Tambah produk</a>
                      ...
                      <tbody>   
                        <?php  
                            ...
                            $sql_isi = "  SELECT DISTINCT
                                            ...
                                            ipsm.id_produk, 
                                            ipsm.qty, 
                                            COALESCE(tpr.kode_produk,  tpe.kode_produk, tpsm.kode_set_marwa, tpse.kode_set_ecat) AS kode_produk, 
                                            COALESCE(tpr.nama_produk, tpe.nama_produk, tpsm.nama_set_marwa, tpse.nama_set_ecat) AS nama_produk, 
                                            COALESCE(mr_reg.nama_merk, mr_ecat.nama_merk, mr_set_mrw.nama_merk, mr_set_ecat.nama_merk) AS nama_merk, 
                                            COALESCE(akl_reg.no_izin_edar, akl_ecat.no_izin_edar, akl_set_mrw.no_izin_edar, akl_set_ecat.no_izin_edar) AS akl, 
                                          ...
                                          LEFT JOIN tb_produk_set_marwa tpsm ON (ipsm.id_produk = tpsm.id_set_marwa)
                                          LEFT JOIN tb_produk_set_ecat tpse ON (ipsm.id_produk = tpse.id_set_ecat)
                                          LEFT JOIN tb_merk mr_reg ON (mr_reg.id_merk = tpr.id_merk)
                                          LEFT JOIN tb_merk mr_ecat ON (mr_ecat.id_merk = tpe.id_merk)
                                          LEFT JOIN tb_merk mr_set_mrw ON (mr_set_mrw.id_merk = tpsm.id_merk)
                                          LEFT JOIN tb_merk mr_set_ecat ON (mr_set_ecat.id_merk = tpse.id_merk)
                                          ...
                                          LEFT JOIN tb_kat_produk akl_set_mrw ON (tpsm.id_kat_produk = akl_set_mrw.id_kat_produk)
                                          LEFT JOIN tb_kat_produk akl_set_ecat ON (tpse.id_kat_produk = akl_set_ecat.id_kat_produk)
                                          WHERE ipsm.id_set_marwa = '$decrypt_id' ORDER BY nama_produk ASC";
                        ...
                        <tr class="<?php echo $non_akl_class; ?>">
                          ...
                          <td class="text-center text-nowrap">
                            <a href="edit-isi-produk-set-marwa.php?edit-id=<?php echo $encrypt_id_isi ?>" class="btn btn-warning btn-sm"><i class="bi bi-pencil"></i></a>
                            <a href="proses/proses-produk-set-marwa.php?hapus-isi-set=<?php echo $encrypt_id_isi ?>&kode=<?php echo $encrypt_id ?>" class="btn btn-danger btn-sm delete-data"><i class="bi bi-trash"></i></a>
                          </td>
                        </tr>
                        <?php $no++; ?>
                        <?php } ?>
