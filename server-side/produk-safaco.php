...
$sql = "SELECT 
            ...
            lok.nama_area,
            lok.no_rak,
            COALESCE(spr.id_produk_safaco, 0) AS id_produk_spr,
            spr.stock
...
$sqlTotal = "SELECT COUNT(*) as total FROM tb_produk_safaco";
$totalResult = mysqli_query($connect, $sqlTotal);
$totalData = mysqli_fetch_assoc($totalResult)['total'];

// Total data setelah filter
$sqlFiltered = "SELECT COUNT(*) as total FROM tb_produk_safaco as pr";
if (!empty($whereClauses)) {
...
while ($row = mysqli_fetch_assoc($query)) {
    $id_produk = encrypt($row['id_produk_safaco'], $key_global);
    $stock = $row['stock'];
    ...
    $data[] = array(
        ...
        '<div class="p-1 text-center text-nowrap"> 
            ...
            <a class="btn btn-warning btn-sm" href="edit-produk-safaco.php?edit-data='.$id_produk.'" title="Edit">
                <i class="bi bi-pencil"></i>
            </a>
            <br>'.$hapusProduk.'
            <a class="btn btn-info btn-sm mt-1" href="cetak-qr-code-safaco.php?id='. $id_produk .'">
