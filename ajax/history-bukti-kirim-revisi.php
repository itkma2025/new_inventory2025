...
if (isset($_POST['id'])) {
    $id_komplain = decrypt($_POST['id'], $key_global); // Dekripsi ID 
    ...
    $sql = $connect->query("
                            SELECT DISTINCT
                                ...
                                hibt.approval_date, 
                                hibt.created_date
                            FROM history_inv_bukti_terima_revisi AS hibt
                            LEFT JOIN inv_komplain AS kmpl ON hibt.id_komplain = kmpl.id_komplain
                            LEFT JOIN inv_nonppn AS nonppn ON kmpl.id_inv = nonppn.id_inv_nonppn
                            LEFT JOIN inv_ppn AS ppn ON kmpl.id_inv = ppn.id_inv_ppn
                            LEFT JOIN inv_bum AS bum ON kmpl.id_inv = bum.id_inv_bum
                            LEFT JOIN revisi_status_kirim sk ON hibt.id_komplain = sk.id_komplain
                            LEFT JOIN inv_penerima_revisi ip ON hibt.id_komplain = ip.id_komplain
                            ...
                            WHERE hibt.id_komplain = '$id_komplain'
