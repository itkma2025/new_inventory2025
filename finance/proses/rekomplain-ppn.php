<?php  
    ...
    if(isset($sanitasi_post['komplain'])){
        ...
        try{
            ...
            $sql_inv_ppn = $connect->query("SELECT ppn.no_inv, kmpl.id_komplain, kmpl.total_komplain
                                                FROM inv_ppn AS ppn 
                                                LEFT JOIN inv_komplain kmpl ON (ppn.id_inv_ppn = kmpl.id_inv)
                                                WHERE ppn.id_inv_ppn = '$id_inv_decrypt'");
            $data_inv = $sql_inv_ppn->fetch_assoc();
            $no_inv = $data_inv['no_inv'];
            $id_komplain = $data_inv['id_komplain'];
            $total_komplain = $data_inv['total_komplain'] + 1;

            // Proses update invoice
            $stmt = $connect->prepare("UPDATE inv_ppn SET status_transaksi = 'Komplain' WHERE id_inv_ppn = ?");
