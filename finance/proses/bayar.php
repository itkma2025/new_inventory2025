...
if(isset($_POST['simpan-pembayaran'])){
    ...
    $tgl_bayar = $sanitasi_post['tgl_bayar'];
    $keterangan_bayar = $sanitasi_post['keterangan_bayar'];
    $jenis_pembayaran = $sanitasi_post['pilih_nominal'];
    $status_potongan = $sanitasi_post['status_potongan'];
    $jumlah_potongan = $sanitasi_post['jumlah_potongan'];
    $total_potongan = str_replace('.', '', $sanitasi_post['nominal_potongan']); // Menghapus tanda ribuan (,)
    $total_potongan = intval($total_potongan); // Mengubah string harga menjadi integer
    $total_bayar = str_replace('.', '', $sanitasi_post['nominal_bayar']); // Menghapus tanda ribuan (,)
    $total_bayar = intval($total_bayar); // Mengubah string harga menjadi integer
    $nama_invoice = 'Invoice_Non_PPN';
    $sisa_tagihan = $sanitasi_post['sisa_tagihan'];
    $nama_pengirim = $sanitasi_post['nama_pengirim'];
    $rek_pengirim = $sanitasi_post['rek_pengirim'];
    $id_bank_pengirim = $sanitasi_post['id_bank_pengirim']; 
    $id_bank_select = $sanitasi_post['id_bank_select'];
    $created_by = decrypt($_SESSION['tiket_nama'], $key_global);
    ...
    if ($metode_bayar == 'cash'){
        try{
            ...
            $stmt = $connect->prepare("INSERT IGNORE INTO finance_bayar 
                                                            (id_bayar, id_bank_pt, id_tagihan, id_finance, id_bukti, metode_pembayaran, keterangan_bayar, jenis_pembayaran, status_potongan, jumlah_potongan, total_potongan, total_bayar, tgl_bayar, created_by) 
                                                            VALUES 
                                                            (?, '-', ?, ?, '-', ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssssiisiiss", $id_bayar, $id_bill_decrypt, $id_finance, $metode_bayar, $keterangan_bayar, $jenis_pembayaran, $status_potongan, $jumlah_potongan, $total_potongan, $total_bayar, $tgl_bayar, $created_by);
            $simpan_bayar = $stmt->execute();

            // Proses update finance
            $stmt = $connect->prepare("UPDATE finance SET status_pembayaran = 1, status_lunas = ?  WHERE id_finance = ?");
        ...
        $id_bank_pt = $sanitasi_post['id_bank_pt'];  
        if($jenis_inv == 'nonppn'){
        ...
        $cek_data = mysqli_query($connect, "SELECT id_bank, no_rekening, atas_nama FROM bank_cs WHERE id_cs = '$id_cs' AND id_bank = '$bank_pengirim' AND no_rekening = '$rek_pengirim' AND atas_nama = '$nama_pengirim'");
        try{
            // Mulai transaksi
            $connect->begin_transaction();
            $update_finance = "";
            if($sisa_tagihan == 0 ){
                $stmt = $connect->prepare("UPDATE finance SET status_pembayaran = 1, status_lunas = 1  WHERE id_finance = ?");
            ...
            $stmt = $connect->prepare("INSERT IGNORE INTO finance_bayar 
                        ...
                        VALUES 
                        (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("sssssssiisiiss", $id_bayar, $id_bank_pt, $id_bill_decrypt, $id_finance, $id_bukti, $metode_bayar, $keterangan_bayar, $jenis_pembayaran, $status_potongan, $jumlah_potongan, $total_potongan, $total_bayar, $tgl_bayar, $created_by);
            $simpan_bayar = $stmt->execute();

            // Proses simpan bukti pembayaran
            $stmt = $connect->prepare("INSERT IGNORE INTO finance_bukti_tf
                                        (id_bukti_tf, id_finance, tf_bank, rek_pengirim, tf_an, bukti_tf, path, created_by) 
                                        VALUES 
                                        (?, ?, ?, ?, ?, ?, ?, ?)");
