    ...
    <main id="main" class="main">
        ...
        <section>
            ...
            <div class="card shadow p-2">
                ...
                <?php
                    $cek_total_inv =  $total_inv;
                    $sp_disc = $data_inv['sp_disc'] / 100;
                    $ongkir = $data_inv['ongkir'];
                    if ($jenis_inv == 'ppn'){
                        $sub_total_spdisc = $sub_total * $sp_disc;
                        $grand_total_fix = $sub_total - $sub_total_spdisc;
                        $ppn_input = 11 / 100;
                        $ppn = $grand_total_fix * $ppn_input;
                        $grand_total_ppn = round($grand_total_fix * 1.11 + $ongkir);
                        if($cek_total_inv != $grand_total_ppn){
                            $connect->query("UPDATE inv_ppn SET total_inv = '$grand_total_ppn' WHERE id_inv_ppn = '$id_inv'"); 
                            ?>
                                <script>
                        ...
                        if($cek_total_inv != $grand_total_fix){
                            $connect->query("UPDATE inv_nonppn SET total_inv = '$grand_total_fix' WHERE id_inv_nonppn = '$id_inv'"); 
                            ?>
                                <script>
                                    window.location.reload();
                                    console.log('Reload Berhasil');
                        ...
                        $grand_total_fix = round($sub_total - $sub_total_spdisc + $ongkir);
                        if($cek_total_inv != $grand_total_fix){
                            $connect->query("UPDATE inv_bum SET total_inv = '$grand_total_fix' WHERE id_inv_bum = '$id_inv'"); 
                            ?>
                                <script>
                                    window.location.reload();
                                    console.log('Reload Berhasil');
