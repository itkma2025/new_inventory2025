          ...
          <?php  
            ...
            while ($data_cb = mysqli_fetch_array($query_cb)) {
                ...
                while($data_trx = mysqli_fetch_array($sql_trx)){
                  ...
                  $tagihanCbBayar = round($hasilCbPajak + $hasilCbTotalInv + $cbPerBarang);
                }
                if ($fnc_total_cb != $tagihanCbBayar) {
                  $simpan_total_cb = $connect->query("UPDATE finance SET total_cb = '$tagihanCbBayar' WHERE id_inv = '$id_inv'");
                  if ($simpan_total_cb) {
                      ?>
                          <script>
                              // window.location.reload();
                              console.log('Reload Berhasil');
                          </script>
                      <?php
                  }
                }
          ...
          <div class="card-body p-3">
            <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist" style="margin-left: 13px;">
              <li class="nav-item" role="presentation">
                <a href="#" class="nav-link active">Tagihan Belum Lunas</a>
              </li>
              <li class="nav-item" role="presentation">
                <a href="list-tagihan-penjualan-lunas.php?date_range=monthly" class="nav-link">Tagihan Lunas</a>
              </li>
            </ul>
            <div class="col">
                <div class="p-3">
                    ...
                    <div class="btn-group">
                        ...
                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                            ...
                            <form action="" method="GET" class="form-group newsletter-group" id="dateForm">
                                ...
                                <a href="list-tagihan-penjualan.php?date_range=year" name="tampilkan" class="custom-dropdown-item dropdown-item rounded bg-danger text-white" id="resetLink">Reset</a>
                            </form>
                            <script>
                                document.addEventListener('DOMContentLoaded', function() {
                                    ...
                                    endDateInput.addEventListener('change', () => {
                                        ...
                                        const newUrl = `list-tagihan-penjualan.php?${queryParams.toString()}`;
                  ...
                  <tbody>
                    <?php  
                      ...
                      $sql = "SELECT  
                              ...
                              HAVING tagihan.total_tagihan != (total_pembayaran + total_potongan) OR SUM(fnc.total_cb) != total_pembayaran_cb
                    ...
                    <tr>
                      ...
                      <td class="text-center text-nowrap"><?php echo date('d/m/Y',strtotime($data['tgl_tagihan']))?></td>
                      ...
                      <td class="text-end">
                        <?php
                          if($total_sisa_tagihan == 0){
                            if($sisa_tagihan_cb != 0){
                              ?>
                                <span class="icon ms-1" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-title="Sisa Tagihan CB">
                                  <?php echo number_format($sisa_tagihan_cb,0,'.','.'); ?>
                                </span>
                              <?php
                            }
                          }else{
                            if($sisa_tagihan_cb != 0){
                              ?>
                                <span class="icon ms-1" 
                                  ...
                                  title="Sisa Tagihan CB : <?php echo number_format($sisa_tagihan_cb,0,'.','.'); ?><br>Sisa Tagihan Invoice : <?php echo number_format($total_sisa_tagihan,0,'.','.'); ?>">
                                  <?php echo number_format($total_tagihan_akhir,0,'.','.'); ?>
                                </span>
                              <?php
                            } else {
                              ?>
                                <span data-bs-toggle="tooltip" 
                                  title="Sisa Tagihan Invoice : <?php echo number_format($total_sisa_tagihan,0,'.','.'); ?>">
                                  <?php echo number_format($total_tagihan_akhir,0,'.','.'); ?>
                                </span>
                              <?php
                            }
...
<script>
 $(document).ready(function() {
    // Inisialisasi Tooltip dengan opsi untuk mendukung HTML
    const tooltipElement = document.querySelector('#tooltipDynamic');
    const tooltip = new bootstrap.Tooltip(tooltipElement, {
        html: true  // Mengaktifkan penggunaan HTML dalam tooltip
    });
  });
</script>
