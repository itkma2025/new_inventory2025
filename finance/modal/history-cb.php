...
<body>
    <?php
        if(isset($_POST['data_id'])){
            $finance_id = htmlspecialchars(decrypt($_POST['data_id'], $key_finance));
            $total_cb = $_POST['data_cb'];
        }
        ...
        $no_tagihan = $data_finance['no_tagihan'];
    ...
    <div class="text-end mt-3 me-2">
        <button class="btn btn-success btn-sm fs-6" id="no_tagihan"></button>
    </div>
    ...
    <div class="row mt-2">
        <?php 
            ...
            $sql_bayar = $connect->query("  SELECT  
                                            ...
                                            LEFT JOIN finance_bayar_cb byr ON (fnc.id_finance = byr.id_finance)
                                            LEFT JOIN inv_nonppn nonppn ON (fnc.id_inv = nonppn.id_inv_nonppn)
                                            LEFT JOIN inv_ppn ppn ON (fnc.id_inv = ppn.id_inv_ppn)
                                            LEFT JOIN inv_bum bum ON (fnc.id_inv = bum.id_inv_bum)
            ...
            $sisa_tagihan = $total_cb - $total_bayar;
        ...
        <div class="col-xl-3">
            <div class="col-12">
                <label class="col-form-label">Total Tagihan Cashback :</label>
            </div>
            <div class="col-12">
                <input type="text" class="form-control text-end" value="<?php echo number_format($total_cb,0,'.','.') ?>" readonly>
            </div>
        </div>
        <div class="col-xl-3">
            <div class="col-12">
            ...
            <tbody>
                <?php  
                    ...
                    while($data_history = mysqli_fetch_array($query_history)){
                        $id_bayar = $data_history['id_bayar_cb'];
                        ...
                        if($metode_pembayaran == 'cash'){
                            ...
                            $no_rek = "-";
                            $atas_nama = "-";
                            $bukti_tf = '<button type="button" class="btn btn-primary btn-sm" data-bs-toggle="modal" disabled>Lihat Gambar</button>';
    ...
    <script>
        $(document).ready(function(){
            ...
            var noTagihan = '<?php echo $no_tagihan ?>';
            $('#no_tagihan').html(noTagihan);
        })
    </script>
</body>
</html>