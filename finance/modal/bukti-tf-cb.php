    ...
    <?php
        if(isset($_POST["data_id"])){
            ...
            $sql_bukti = mysqli_query($connect, "SELECT 
                                                        fnc.jenis_inv, fnc.id_inv, fnc.id_tagihan, byr.id_bayar_cb, byr.id_bukti, bkt.tf_bank, bkt.rek_penerima, bkt.tf_an, bkt.bukti_tf, bnk.nama_bank 
                                                FROM finance AS fnc 
                                                LEFT JOIN finance_bayar_cb byr ON (fnc.id_finance = byr.id_finance)
                                                LEFT JOIN finance_bukti_tf_cb bkt ON (bkt.id_bukti_tf = byr.id_bukti)
                                                LEFT JOIN bank bnk ON (bnk.id_bank = bkt.tf_bank)
                                                WHERE byr.id_bayar_cb = '$id_bayar'");
            $data_bukti = mysqli_fetch_array($sql_bukti);
            $id_inv = $data_bukti['id_inv'];
            $an = $data_bukti['tf_an'];
            $rek = $data_bukti['rek_penerima'];
                            ...
                            <div class="row g-0">
                                ...
                                <div class="col-md-6">
                                    <div class="card-body">
                                        ...
                                        <p class="card-text" style="font-size: 16px">Nama Penerima : '.$an.'</p>
                                        <p class="card-text" style="font-size: 16px">Rek Penerima : '.$rek.'</p>
                                        <p class="card-text" style="font-size: 16px">Bank Penerima : '.$bank.'</p>
                                    </div>
                                </div>
                            </div>
                        </div>  
