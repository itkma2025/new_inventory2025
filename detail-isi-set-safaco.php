      ...
      <div class="container-fluid">
        <div class="card">
          <div class="card-body p-3">
            ...
            $sql = "SELECT 
                      tbsm.id_set_safaco,
                      tbsm.kode_set_safaco,
                      tbsm.nama_set_safaco,
                      tbsm.harga_set_safaco,
                      lok.nama_lokasi,
                      lok.no_lantai,
                      lok.nama_area,
                      lok.no_rak
                    FROM tb_produk_set_safaco AS tbsm 
                    LEFT JOIN tb_lokasi_produk AS lok ON (tbsm.id_lokasi = lok.id_lokasi)
                    WHERE id_set_safaco = '$decrypt_id'";
            ...
            <?php  
              if($cek_data > 0){
                ?>
                  <div class="row">
                    <div class="col-sm-8">
                      <p>Kode set : <?php echo $data['kode_set_safaco']; ?></p>
                      <p>Nama set : <?php echo $data['nama_set_safaco']; ?></p>
                      <?php
                      include "koneksi.php";
                      $grand_total = 0;
                      $sql_data = "SELECT 
                      ...
                      <p>Harga Jual : <?php echo number_format($data['harga_set_safaco'], 0, '.', '.'); ?></p>
                      <p>Lokasi : <?php echo $data['nama_lokasi']; ?> / <?php echo $data['no_lantai']; ?> / <?php echo $data['nama_area']; ?> / <?php echo $data['no_rak']; ?></p>
                    </div>
                    <div class="col-sm-4 text-end">
                      <a href="tambah-isi-produk-set-safaco.php?id-set=<?php echo $encrypt_id ?>" class="btn btn-primary"><i class="bi bi-plus-circle"></i> Tambah produk</a>
                      <a href="data-produk-set-safaco.php" class="btn btn-secondary"><i class="bi bi-arrow-left"></i> Kembali</a>
                    </div>
                  </div>
                      ...
                      <tbody>   
                        <?php  
                          ...
                          $sql_isi = " SELECT DISTINCT
                                          ipse.id_isi_set_safaco, 
                                          ipse.id_set_safaco, 
                                          ipse.id_produk, 
                                          ipse.qty, 
                                          COALESCE(tpr.kode_produk,  tpe.kode_produk) AS kode_produk, 
                                          COALESCE(tpr.nama_produk,  tpe.nama_produk) AS nama_produk, 
                                          COALESCE(mr_reg.nama_merk,  mr_safaco.nama_merk) AS nama_merk, 
                                          COALESCE(akl_reg.no_izin_edar, akl_safaco.no_izin_edar) AS akl, 
                                          COALESCE(tpr.harga_produk, tpe.harga_produk) AS harga_produk
                                        FROM isi_produk_set_safaco ipse
                                        LEFT JOIN tb_produk_reguler tpr ON (ipse.id_produk = tpr.id_produk_reg)
                                        LEFT JOIN tb_produk_safaco tpe ON (ipse.id_produk = tpe.id_produk_safaco)
                                        LEFT JOIN tb_merk mr_reg ON (mr_reg.id_merk = tpr.id_merk)
                                        LEFT JOIN tb_merk mr_safaco ON (mr_safaco.id_merk = tpe.id_merk)
                                        LEFT JOIN tb_kat_produk akl_reg ON (tpr.id_kat_produk = akl_reg.id_kat_produk)
                                        LEFT JOIN tb_kat_produk akl_safaco ON (tpe.id_kat_produk = akl_safaco.id_kat_produk)
                                        WHERE ipse.id_set_safaco = '$decrypt_id'";
                          ...
                          while($data_produk = mysqli_fetch_array($query_isi)){
                            ...
                            $id_isi_set_marwa = $data_produk['id_isi_set_safaco'];  
                        ...
                        <tr class="<?php echo $non_akl_class; ?>">
                          ...
                          <td class="text-center text-nowrap">
                            <a href="edit-isi-produk-set-safaco.php?edit-id=<?php echo $encrypt_id_isi ?>" class="btn btn-warning btn-sm"><i class="bi bi-pencil"></i></a>
                            <a href="proses/proses-produk-set-safaco.php?hapus-isi-set=<?php echo $encrypt_id_isi ?>&kode=<?php echo $encrypt_id ?>" class="btn btn-danger btn-sm delete-data"><i class="bi bi-trash"></i></a>
                          </td>
                        </tr>
                        <?php $no++; ?>
                        <?php } ?>
