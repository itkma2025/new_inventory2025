...
<body>
    ...
    <main id="main" class="main">
        ...
        <section>
            <?php
                ...
                if($jenis_inv == 'nonppn'){
                    ...
                    if($status_transaksi_inv != 'Dikirim'){
                        ?>
                            <script>
                        ...
                        <div class="card-body p-3 border" style="min-height: 234px;">
                            ...
                            <?php
                                ...
                                if ($penerima) {
                                    ...
                                    // Cek apakah data yang diambil valid atau tidak null
                                    $nama_penerima = "";
                                    if ($data_penerima && isset($data_penerima['nama_penerima']) && $data_penerima['nama_penerima'] != "") {
                                        $nama_penerima = $data_penerima['nama_penerima'];
                                    }
                                } else {
                                    // Query gagal, tampilkan pesan error atau tangani sesuai kebutuhan
                            ...
                            <?php
                                if ($jenis_pengiriman == 'Ekspedisi') {
                                    ...
                                    } else if($data_status_kirim['jenis_penerima'] == "" && $jenis_pengiriman == 'Driver'){
                                        ?>
                                            <div class="row mt-2">
                                                <div class="col-5">
                                                    <p style="float: left;">Jenis Pengiriman</p>
                ...
                <div class="row mt-3">
                    <div class="col-md-12">
                        <div class="text-start">
                            <a href="invoice-reguler-dikirim.php?sort=baru" class="btn btn-warning btn-detail mb-2 btn-mobile">
                                <i class="bi bi-arrow-left"></i>
                                Halaman Sebelumnya
                            </a> 
                            <?php
                                if ($role == "Super Admin" || $role == "Admin Penjualan") {
                                    $sql_bukti_kirim = "SELECT id_inv FROM inv_bukti_terima WHERE id_inv = '$id_inv'";
                                    $query_bukti_kirim = mysqli_query($connect, $sql_bukti_kirim);
                                    $total_row = mysqli_num_rows($query_bukti_kirim);
                                    $cek_jenis_pengiriman = $data_cek['jenis_pengiriman'];
                                    $cek_jenis_penerima = $data_cek['jenis_penerima'];
                                    $btn_bukti_kirim = '<button class="btn btn-primary btn-detail mb-2 btn-mobile btn-mobile" data-bs-toggle="modal" data-bs-target="#buktiKirim">
                                                            <i class="bi bi-file-earmark-image"></i> Bukti Kirim
                                                        </button>';

                                    $btn_diterima_ex = ' <button class="btn btn-secondary btn-detail mb-2 me-1 btn-mobile btn-mobile" data-bs-toggle="modal"data-bs-target="#DiterimaEx">
                                                            <i class="bi bi-send"></i>
                                                            Diterima
                                                        </button>';

                                    $btn_edit_ongkir_resi = '<button class="btn btn-warning btn-detail mb-2 me-1 btn-mobile btn-mobile" data-bs-toggle="modal" data-bs-target="#editOngkir">
                                                                <i class="bi bi-pencil"></i>
                                                                Edit Ongkir Dan No. Resi
                                                            </button>';

                                    $btn_ubah_driver = '<button type="button" class="btn btn-info mb-2 btn-mobile" data-bs-toggle="modal" data-bs-target="#ubahDriver">
                                                        <i class="bi bi-arrow-repeat"></i> Ubah Driver
                                                    </button>';

                                    $btn_diambil_langsung = '<button type="button" class="btn btn-secondary mb-2 btn-mobile" data-bs-toggle="modal" data-bs-target="#diambil">
                                                                <i class="bi bi-arrow-repeat"></i> Diambil Oleh
                                                            </button>';

                                    
                                    if ($cek_jenis_pengiriman == 'Ekspedisi' && $cek_jenis_penerima == 'Ekspedisi') {
                                        if($no_resi != ''){
                                            echo $btn_bukti_kirim;
                                            echo $btn_diterima_ex;
                                        }
                                        echo $btn_edit_ongkir_resi;
                                    } else if ($total_row > 0 && $cek_jenis_pengiriman == 'Driver' && $data_cek['jenis_penerima'] == 'Ekspedisi'){
                                        echo $btn_bukti_kirim;
                                        echo $btn_diterima_ex;
                                    } else if ($data_cek['jenis_pengiriman'] == 'Driver' && $data_cek['jenis_penerima'] == '') {
                                        echo $btn_ubah_driver;
                                    } else if ($data_cek['jenis_pengiriman'] == 'Diambil Langsung' && $data_cek['jenis_penerima'] == 'Customer'){
                                        echo $btn_diambil_langsung;
                                    }
                                ?>
                                <!-- Ubah Jenis Pengiriman -->
                                <a href="proses/proses-ubah-pengiriman.php?id_status_kirim=<?php echo encrypt($id_status_kirim, $key_global) ?>&&id_inv=<?php echo encrypt($id_inv, $key_global) ?>" class="btn btn-primary mb-2 btn-mobile update-data">
        ...
        <?php include "modal/ubah-driver-dikirim.php"; ?>
    </main>
    <?php include "modal/edit-ongkir-resi-dikirim.php"; ?>
    <!-- Footer -->
