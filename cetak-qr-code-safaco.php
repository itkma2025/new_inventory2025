...
$sql_qr = $connect->query("SELECT id_link FROM qr_link_safaco");
...
$outputDir = "gambar/QRcode-safaco/"; // Pastikan folder ini ada dan dapat diakses
...
if ($cek_data > 0) {
    ...
    if ($server_saat_ini != $domain) {
        ...
        $url_qr = $server_saat_ini . "/detail-produk-safaco.php?id=" . encrypt($id_produk_qr, $key);
        ...
        $sql_update_qr = $connect->query("UPDATE qr_link_safaco SET url_qr = '$url_qr', qr_img = '$new_nama_qr_img', updated_date = '$today' WHERE id_produk_qr = '$id_produk_qr'");

        if ($sql_update_qr) {
            header("Location:tampil-qr-code-safaco.php?id=" . encrypt($id_produk_qr, $key));
        }
    } else {
        header("Location:tampil-qr-code-safaco.php?id=" . encrypt($id_produk_qr, $key));
    }
    ...
    $sql_produk = $connect->query("SELECT id_produk_safaco, nama_produk, kode_produk FROM tb_produk_safaco WHERE id_produk_safaco = '$id'");
    $data_produk = mysqli_fetch_array($sql_produk);

    $id_produk = $data_produk['id_produk_safaco'];
    ...
    $url_qr = $server_saat_ini . "/detail-produk-safaco.php?id=" . $encrypt_id_produk;
    ...
    // Simpan ke database
    $simpan_qr = $connect->query("INSERT INTO qr_link_safaco (id_produk_qr, url_qr, qr_img) VALUES ('$id_produk', '$url_qr', '$new_nama_qr_img')");

    if ($simpan_qr) {
        header("Location:tampil-qr-code-safaco.php?id=" . encrypt($id_produk, $key));
    }
}
