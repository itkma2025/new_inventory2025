<?php  
    ...
    if($action === 'approval'){
		...
		$approval = 2;
		// Mulai Transaksi
		$connect_ecat->begin_transaction();
		try {
			// Update status kirim
			$stmt = $connect_ecat->prepare("UPDATE status_kirim
														SET
															status_review = ?,
															review_date = ?	
														WHERE id_inv_ecat = ?
			...
			// Update bukti terima
			$stmt = $connect_ecat->prepare("UPDATE inv_bukti_terima
										SET
											approval = ?,
											approval_date = ?,
											approval_by = ?,
			...
			$stmt = $connect_ecat->prepare("INSERT INTO history_inv_bukti_terima   
													(id_history, id_bukti_terima, id_inv, bukti_terima, lokasi, approval, approval_date, approval_by, alasan, created_date, created_by)
										SELECT ?, id_bukti_terima, id_inv_ecat, bukti_terima, lokasi, approval, approval_date, approval_by, alasan, created_date, created_by 
			...
			$connect_ecat->rollback();
			$_SESSION['info'] = "Data Gagal Disimpan";
			header("Location: {$_SERVER['HTTP_REFERER']}");
            exit();
		}
    } else if ($action === 'reject') {
		$id_inv = decrypt($sanitasi_get['id'], $key_global);
		...
		$status_review = 1;
		$approval = 1;
		$sql_status_kirim = $connect_ecat->query("SELECT jenis_penerima FROM status_kirim WHERE id_inv_ecat = '$id_inv'");
		...
		try {
			...
			$query_status_kirim .= " WHERE id_inv_ecat = ?";
			$params[] = $id_inv;
			...
			// Prepare dan bind
			$stmt = $connect_ecat->prepare($query_status_kirim);
			...
			// insert history bukti kirim
			$stmt = $connect_ecat->prepare("INSERT INTO history_inv_bukti_terima   
													(id_history, id_bukti_terima, id_inv, bukti_terima, lokasi, approval, approval_date, approval_by, jenis_reject, alasan, created_date, created_by)
										SELECT ?, id_bukti_terima, id_inv_ecat, bukti_terima, lokasi, approval, approval_date, approval_by, jenis_reject, alasan, created_date, created_by 
										FROM inv_bukti_terima
										WHERE id_inv_ecat = ?");
			// Bind parameter dan eksekusi
			$stmt->bind_param("ss", $id_history, $id_inv);
