<?php
    ...
    if (isset($_POST['simpan-inv'])) {
        ...
        $id_inv_bum = $sanitasi_post['id_inv_bum'];
        $id_inv_bum_encrypt = encrypt($id_inv_bum, $key_global);
        $id_cb_bum = $sanitasi_post['id_cb_bum'];
        $no_inv_bum = $sanitasi_post['no_inv_bum'];
        ...
        try {
            ...
            // Simpan data invoice
            $stmt = $connect->prepare("INSERT IGNORE INTO inv_bum 
                                        (id_inv_bum, no_inv, tgl_inv, cs_inv, tgl_tempo, sp_disc, kwitansi, surat_jalan, note_inv, kategori_inv, ongkir, status_transaksi, nama_invoice, created_by)
                                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssssssssisss", $id_inv_bum, $no_inv_bum, $tgl_inv, $cs_inv, $tgl_tempo, $sp_disc, $kwitansi, $surat_jalan, $note_inv, $jenis_inv, $ongkir, $status_inv, $nama_invoice, $created_by);
            ...
            $stmt = $connect->prepare("INSERT IGNORE INTO cashback_bum 
                                        (id_cb_bum, id_inv, status_cb, jenis_cb, cb_total_inv, cb_pajak, created_by)
                                        VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssisiis", $id_cb_bum, $id_inv_bum, $status_cb, $selected_cashback_decrypt, $cb_total_inv, $cb_pajak, $created_by);
            ...
            for ($i = 0; $i < $id_spk_count; $i++) {
                ...
                $stmt_spk->bind_param("ssss", $id_inv_bum, $no_po, $status_spk, $id_spk_array_decrypt);
            ...
            header("Location:../detail-produk-proforma.php?jenis=bum&id=$id_inv_bum_encrypt");
            exit();
        } catch (Exception $e) {
        ...
        try {
            ...
            if (empty($kat_inv)){
                if (!empty($sp_disc_edit)){
                    $stmt = $connect->prepare("UPDATE inv_bum SET sp_disc = ? WHERE id_inv_bum = ?");
                    ...
                    if($update_inv){
                        ...
                        $connect->commit();
                        $_SESSION['info'] = "Disimpan";
                        header("Location:../detail-produk-proforma.php?jenis=bum&&id=$id_inv");
                    } else {
                        $connect->rollback();
                        throw new Exception(); 
                    }
                ...
                $stmt = $connect->prepare("UPDATE inv_bum SET kategori_inv = ?, sp_disc = ? WHERE id_inv_bum = ?");
    ...
    // ======================================================================================
        ...
        if ($jenis_pengiriman == 'Driver') {
            ...
            try {
                ...
                // Update Invoice
                $stmt = $connect->prepare("UPDATE inv_bum SET status_transaksi = 'Dikirim' WHERE id_inv_bum = ?");
                $stmt->bind_param('s', $id_inv_decrypt);
                $update_inv = $stmt->execute();
    ...
    // ======================================================================================
        ...
        try {
            ...
            if($status_cb == '0'){
                $stmt = $connect->prepare("UPDATE cashback_bum SET status_cb = ?, jenis_cb = '', cb_total_inv = '', cb_pajak = '', updated_date = ?, updated_by = ? WHERE id_inv = ?");
                $stmt->bind_param("ssss", $status_cb, $updated_date, $updated_by, $id_inv_decrypt);
                $update_status_cb = $stmt->execute();
            } else {
                $stmt = $connect->prepare("UPDATE cashback_bum SET status_cb = ?, updated_date = ?, updated_by = ? WHERE id_inv = ?");
                $stmt->bind_param("ssss", $status_cb, $updated_date, $updated_by, $id_inv_decrypt);
                $update_status_cb = $stmt->execute();
            }

            $stmt = $connect->prepare("UPDATE inv_bum SET cs_inv = ?, alamat_inv = ?, updated_date = ?, updated_by = ? WHERE id_inv_bum = ?");
            $stmt->bind_param("sssss", $cs_inv, $alamat, $updated_date, $updated_by, $id_inv_decrypt);
            $update_inv = $stmt->execute();
    ...
    // ======================================================================================
        ...
        $stmt = $connect->prepare("UPDATE cashback_bum 
                                    SET 
                                        jenis_cb = ?, 
                                        cb_total_inv = ?, 
                                        cb_pajak = ?, 
