<?php
    ...
    if (isset($_POST['diambil-oleh'])) {
        ...
        if (!isset($sanitasi_post['nonce_token']) || $sanitasi_post['nonce_token'] !== $_SESSION['nonce_token']) {
            ...
            header("Location:../detail-produk-dikirim.php?jenis=bum&&id=$id_inv");
        }
        ...
        try {
            ...
            // Proses update invoice penerima
            $stmt = $connect->prepare("UPDATE inv_bum SET status_transaksi = 'Diterima' WHERE id_inv_bum = ?");
            $stmt->bind_param("s", $id_inv_decrypt);
            $update_inv_bum = $stmt->execute();
