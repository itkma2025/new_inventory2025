    ...
    if ($status_kirim == 'dikirim') {
        ...
        if ($jenis_pengiriman == 'Driver') {
            ...
            try {
                ...
                $stmt = $connect->prepare("UPDATE inv_ppn SET status_transaksi = 'Komplain Dikirim' WHERE id_inv_ppn = ?");
                ...
                header("Location:../detail-komplain-revisi-ppn.php?id=$id_komplain_encrypt");
                exit();
            } catch (Exception $e) {
            ...
            try {
                ...
                // Prepare statement before update data invoice
                $stmt = $connect->prepare("UPDATE inv_ppn SET status_transaksi = 'Komplain Diambil' WHERE id_inv_ppn = ?");
                $stmt->bind_param('s', $id_inv);
                $update_inv = $stmt->execute();
        ...
        try {
            ...
            // Update status transaksi di tabel inv_ppn
            $update_inv_stmt = $connect->prepare("UPDATE inv_ppn SET status_transaksi = 'Komplain Selesai' WHERE id_inv_ppn = ?");
            $update_inv_stmt->bind_param('s', $id_inv);
            $update_inv_stmt->execute();
            if (!$update_inv_stmt) {
                throw new Exception("Gagal update inv_ppn: " . $update_inv_stmt->error);
            }
        ...
        try {
            ...
            // Prepare statement before update data invoice
            $stmt = $connect->prepare("UPDATE inv_ppn SET status_transaksi = 'Cancel Order' WHERE id_inv_ppn = ?");
            $stmt->bind_param('s', $id_inv);
            $update_inv = $stmt->execute();
