      ...
      <div class="container-fluid">
        <div class="card">
          ...
          <div class="card-body p-3">
            <form action="proses/proses-produk-set-safaco.php" method="post">
              <?php
                $id_set = $_GET['id-set'];
                $UUID = generate_uuid();
                $key = "KM@2024?SET";
              ...
              ?>
              <input type="hidden" class="form-control" name="id_isi_set_safaco" value="BR-SET-ECAT-<?php echo $UUID ?>">
              <input type="hidden" class="form-control" name="id_set_safaco" value="<?php echo $decrypt_id ?>">
              <div class="mb-3">
                <div class="row">
                  <div class="col-sm-6">
                  ...
                  <div class="mt-3">
                    <button type="submit" class="btn btn-primary btn md" name="simpan-isi-set-safaco" id="simpan" disabled><i class="bx bx-save"></i> Simpan</button>
                    <a href="detail-isi-set-safaco.php?detail-id=<?php echo $id_set ?>" class="btn btn-secondary btn md"><i class="bi bi-x"></i> Batal</a>
                  </div>
                </div>
              </div>
            </form>
      ...
      <div class="modal-body">
        <ul class="nav nav-pills mb-3" id="pills-tab" role="tablist">
          <li class="nav-item" role="presentation">
            <button class="nav-link active" id="produk-safaco-tab" data-bs-toggle="pill" data-bs-target="#produk-safaco" type="button" role="tab" aria-controls="produk-safaco" aria-selected="false">Produk E-Cat</button>
          </li>
          <li class="nav-item" role="presentation">
        ...
        <div class="tab-content" id="pills-tabContent">
          <div class="tab-pane fade show active" id="produk-safaco" role="tabpanel" aria-labelledby="pills-home-tab" tabindex="0">
            <div class="table-responsive">
              ...
              <thead>
                ...
                <tbody>
                  ...
                  $no = 1;
                  $sql = "SELECT 
                              COALESCE(tpe.id_produk_safaco, tpse.id_set_safaco) AS id_produk,
                              COALESCE(tpe.kode_produk, tpse.kode_set_safaco) AS kode_produk,
                              COALESCE(tpe.nama_produk, tpse.nama_set_safaco) AS nama_produk,
                              COALESCE(mr_tpe.nama_merk, mr_tpse.nama_merk) AS nama_merk,
                              spe.id_stock_prod_safaco,
                              spe.stock,
                              tkp.min_stock, 
                              tkp.max_stock,
                              SUBSTRING(COALESCE(tpe.id_produk_safaco, tpse.id_set_safaco), 1, 2) AS substr_id_produk
                          FROM tb_produk_safaco AS tpe
                          LEFT JOIN stock_produk_safaco AS spe ON (tpe.id_produk_safaco = spe.id_produk_safaco)
                          LEFT JOIN tb_kat_penjualan AS tkp ON (tkp.id_kat_penjualan = spe.id_kat_penjualan)
                          LEFT JOIN tb_produk_set_safaco AS tpse ON (tpse.id_set_safaco = spe.id_produk_safaco)
                          LEFT JOIN tb_merk AS mr_tpe ON (tpe.id_merk = mr_tpe.id_merk)
                          LEFT JOIN tb_merk AS mr_tpse ON (tpse.id_merk = mr_tpse.id_merk)
                          WHERE SUBSTRING(COALESCE(tpe.id_produk_safaco, tpse.id_set_safaco), 1, 2) = 'BR'
                          ORDER BY nama_produk ASC";
                  $query = mysqli_query($connect, $sql);
                  while ($data = mysqli_fetch_array($query)) {
                    $id_stock = base64_encode($data['id_stock_prod_safaco']);
