...
$sql = "SELECT 
        ...
        LEFT JOIN qr_link_set_ecat qr ON (tpsm.id_set_ecat = qr.id_set_qr)
...
$sql_qr = $connect->query("SELECT id_link FROM qr_link_set_ecat");
...
$outputDir = "gambar/QRcode-set-ecat/"; // Pastikan folder ini ada dan dapat diakses
...
if ($cek_data > 0) {
    ...
    $id_set_qr = $data['id_set_ecat'];
    $img = $data['qr_img'];
    $url = $data['url_qr'];
    $kode_produk = $data['kode_set_ecat']; // Mendapatkan kode produk
    ...
    $nama_produk_sanitized = preg_replace('/[^\w]+/', '_', $data['nama_set_ecat']);
    ...
    if ($server_saat_ini != $domain) {
        ...
        $url_qr = $server_saat_ini . "/detail-produk-set-ecat.php?id=" . encrypt($id_set_qr, $key);
        ...
        $sql_update_qr = $connect->query("UPDATE qr_link_set_ecat SET url_qr = '$url_qr', qr_img = '$new_nama_qr_img', updated_date = '$today' WHERE id_set_qr = '$id_set_qr'");

        if ($sql_update_qr) {
            header("Location:tampil-qr-set-ecat.php?id=" . encrypt($id_set_qr, $key));
        }
    } else {
        header("Location:tampil-qr-set-ecat.php?id=" . encrypt($id_set_qr, $key));
    }
    ...
    $sql_produk = $connect->query("SELECT id_set_ecat, nama_set_ecat, kode_set_ecat FROM tb_produk_set_ecat WHERE id_set_ecat = '$id'");
    $data_produk = mysqli_fetch_array($sql_produk);

    $id_produk = $data_produk['id_set_ecat'];
    $encrypt_id_produk = encrypt($id_produk, $key);
    $nama_produk = $data_produk['nama_set_ecat'];
    $kode_produk = $data_produk['kode_set_ecat'];
    ...
    $url_qr = $server_saat_ini . "/detail-produk-set-ecat.php?id=" . $encrypt_id_produk;
    ...
    // Simpan ke database
    $simpan_qr = $connect->query("INSERT INTO qr_link_set_ecat (id_set_qr, url_qr, qr_img) VALUES ('$id_produk', '$url_qr', '$new_nama_qr_img')");

    if ($simpan_qr) {
        header("Location:tampil-qr-set-ecat.php?id=" . encrypt($id_produk, $key));
    }
}
