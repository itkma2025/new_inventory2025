      ...
      <div class="container-fluid">
        <div class="card">
          <div class="card-header text-center">
            <h4>Tambah Data Stock Produk Safaco</h4>
          </div>
          <div class="card-body p-3">
            <form action="proses/proses-stock-safaco.php" method="POST">
              <?php
                $UUID = generate_uuid();
              ?>
              <input type="hidden" class="form-control" name="id_stock_safaco" value="STOCK_SKM_<?php echo $UUID; ?>">
              <div class="row">
              ...
              <div class="mb-3 pt-3 text-end">
                <button type="submit" name="simpan-stock-safaco" class="btn btn-primary btn-md" id="simpan" disabled><i class="bx bx-save"></i> Simpan Data</button>
                <a href="stock-produk-safaco.php" class="btn btn-secondary btn-md"><i class="bi bi-x"></i> Tutup</a>
              </div>
            </form>
          </div>
    </section>
              ...
              <div class="table-responsive">
                <table class="table table-striped table-bordered" id="table2">
                  ...
                  <tbody>
                    ...
                    include "koneksi.php";
                    $no = 1;
                    $sql = "SELECT  pr.id_produk_safaco AS 'produk_id',
                                    pr.kode_produk,
                                    pr.nama_produk,
                                    pr.id_merk,
                                    pr.id_kat_penjualan,
                            ...
                            LEFT JOIN stock_produk_safaco spr ON (pr.id_produk_safaco = spr.id_produk_safaco)
              ...
              <div class="table-responsive">
                  <table class="table table-striped table-bordered" id="table3">
                    ...
                    <tbody>
                      ...
                      $no = 1;
                      $sql_set = "SELECT 
                                    tpsm.id_set_safaco AS 'produk_id',
                                    tpsm.kode_set_safaco,
                                    tpsm.nama_set_safaco,
                                    tpsm.id_merk,
                                    tpsm.id_kat_penjualan,
                                    mr.nama_merk,
                                    spr.stock
                                FROM tb_produk_set_safaco as tpsm
                                LEFT JOIN tb_merk mr ON (tpsm.id_merk = mr.id_merk)
                                LEFT JOIN stock_produk_safaco spr ON (tpsm.id_set_safaco = spr.id_produk_safaco)
                                WHERE register_value = '0' ";
                      $query_set = mysqli_query($connect, $sql_set);
                      while ($data_set = mysqli_fetch_array($query_set)) {
                      ?>
                        <tr data-idprod="<?php echo $data_set['produk_id']; ?>" data-namaprod="<?php echo $data_set['nama_set_safaco']; ?>" data-merkid="<?php echo $data_set['id_merk']; ?>" data-katjualid="<?php echo $data_set['id_kat_penjualan']; ?>" data-merkprod="<?php echo $data_set['nama_merk']; ?>" data-bs-dismiss="modal">
                            <td class="text-center text-nowrap"><?php echo $no; ?></td>
                            <td class="text-center text-nowrap"><?php echo $data_set['kode_set_safaco']; ?></td>
                            <td class="text-start text-nowrap"><?php echo $data_set['nama_set_safaco']; ?></td>
