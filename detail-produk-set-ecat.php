...
<body>
    <?php
        ...
        $sql = "SELECT 
                    ...
                    tpse.gambar,
                    tpse.deskripsi,
                    DATE_FORMAT(tpse.created_date, '%d/%m/%Y, %H:%i:%s') AS produk_created,  -- Format tanggal Indonesia
                    CASE 
                        WHEN tpse.updated_date = '0000-00-00 00:00:00' THEN '-'
                        ELSE DATE_FORMAT(tpse.updated_date, '%d/%m/%Y, %H:%i:%s')
                    END AS produk_updated,
                    tpse.created_by,
                    tpse.updated_by,
                    tpse.id_set_ecat as produk_id,
                    ...
                    lok.no_rak,
                    gr.nama_grade
                FROM tb_produk_set_ecat as tpse
                LEFT JOIN $database2.user AS uc ON (tpse.created_by = uc.id_user)
                LEFT JOIN $database2.user AS uu ON (tpse.updated_by = uu.id_user)
                LEFT JOIN tb_merk mr ON (tpse.id_merk = mr.id_merk)
                LEFT JOIN tb_kat_produk kp ON (tpse.id_kat_produk = kp.id_kat_produk)
                LEFT JOIN tb_kat_penjualan kj ON (tpse.id_kat_penjualan = kj.id_kat_penjualan)
                LEFT JOIN tb_lokasi_produk lok ON (tpse.id_lokasi = lok.id_lokasi)
                LEFT JOIN tb_produk_grade gr ON (tpse.id_grade = gr.id_grade)
                WHERE tpse.id_set_ecat ='$id_set_ecat'";
        $result = mysqli_query($connect, $sql);
        $data = mysqli_fetch_array($result);
        $img = "";
        if ($data['gambar'] && file_exists("gambar/upload-produk-set-ecat/" . $data['gambar'])) {
            $img = "gambar/upload-produk-set-ecat/" . $data['gambar'];
        } else {
                ...
                <div class="row mt-3">
                    <div class="col-md-6">
                        <div class="d-flex justify-content-center">
                            <?php  
                                $sql_produk_reg = $connect->query("SELECT id_produk, qty_in, qty_out FROM kartu_stock_set_ecat WHERE id_produk = '$id_set_ecat'");
                                $total_qty_in = 0;
                                $total_qty_out = 0;
                                $stock_tersedia = 0;
                    ...
                    <div class="col-md-6">
                        <div class="d-flex justify-content-end mt-4">
                            <?php  
                                ...
                                $session_url = "";
                                if ($role == "Driver") {
                                    $session_url = "driver/kartu-persediaan-barang.php?jenis_produk=set_ecat&id=" . $id;
                                } else if ($role == "Finance") {
                                    header("Location:404.php");
                                } else {
                                    $session_url = "kartu-persediaan-barang.php?jenis_produk=set_ecat&id=" . $id;
                                }
                        ...
                        <p class="mt-3 custom-margin" style="font-size: 16px;"><?php echo $data['nama_grade'] ?> / <?php echo $data['kat_prod'] ?> / <?php echo $data['kode_set_ecat']; ?></p>
                        <p class="fw-bold custom-margin" style="font-size: 19px;"><?php echo $data['nama_merk']; ?> / <?php echo $data['nama_set_ecat']; ?> / <?php echo $data['no_izin_edar'] ?></p>
                        <p class="fw-bold mt-4" style="font-size: 19px;">Rp. <?php echo number_format($data['harga_set_ecat'],0,'.','.'); ?>,-</p>
