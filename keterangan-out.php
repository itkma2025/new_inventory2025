            ...
            <table class="table table-striped table-bordered" id="table1">
                ...
                <tbody>
                  ...
                  $sql = "SELECT 
                              ket_out.id_ket_out,
                              ket_out.ket_out,
                              ket_out.status_aktif,
                              ket_out.created_date,
                              ket_out.created_by,
                              us_created.nama_user AS user_created
                            FROM keterangan_out AS ket_out
                            LEFT JOIN $database2.user us_created ON (ket_out.created_by = us_created.id_user)
                            ORDER BY ket_out ASC";
                  $query = mysqli_query($connect, $sql) or die(mysqli_error($connect));
                  while ($data = mysqli_fetch_array($query)) {
                    $id_ket_out = encrypt($data['id_ket_out'], $key_global);
                  ...
                  ?>
                    <tr>
                      ...
                      <td class="text-start"><?php echo $data['ket_out'] ?></td>
                      ...
                      <td class="text-nowrap text-center">
                        <div class="form-check form-switch" style="display: inline-block !important;"
                          ...
                          <input class="form-check-input" type="checkbox" role="switch"
                            id="flexSwitchCheckChecked<?php echo $id_ket_out; ?>" <?php echo $checked ?>
                            data-id="<?php echo $id_ket_out; ?>">
                        </div>
                      </td>
                      <td class="text-center text-nowrap">
                        <button class="btn btn-warning btn-sm" data-bs-toggle="modal" data-bs-target="#modal2"
                          data-id="<?php echo encrypt($data['id_ket_out'], $key_global); ?>" data-nama="<?php echo $data['ket_out']; ?>">
                          <i class="bi bi-pencil"></i>
                        </button>
                        <a href="proses/proses-ket-out.php?hapus-ket-out=<?php echo encrypt($data['id_ket_out'], $key_global) ?>"
                          ...
                          <div class="modal-content">
                            ...
                            <form action="proses/proses-ket-out.php" method="POST">
                              ...
                              <div class="modal-footer">
                                <button type="submit" name="edit-ket-out" id="simpan" disabled
...
<div class="modal fade" id="modal1" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h1 class="modal-title fs-5">Tambah Data Keterangan Barang Keluar</h1>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <form action="proses/proses-ket-out.php" method="POST">
          <div class="modal-body">
            <div class="mb-3">
              ...
              ?>
              <div class="mb-3">
                <label class="form-label">Nama Keterangan Barang Keluar</label>
...
<script>
  $(document).ready(function() {
    ...
    $(document).on('change', '.form-check-input[type="checkbox"]', function() {
      ...
      $.ajax({
        url: 'proses/update-status-ket-out.php',
        type: 'POST',
        data: {
          id_ket_out: id,
