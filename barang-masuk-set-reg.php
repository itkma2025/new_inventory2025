                            ...
                            <table class="table table-striped table-bordered" id="table1">
                                <thead>
                                    <tr class="text-white" style="background-color: #051683;">
                                        ...
                                        <td class="text-center text-nowrap p-3" style="width: 80px">Qty</td>
                                ...
                                <tbody>
                                    ...
                                    if(isset($_GET['date_range']))
                                    {
                                        ...
                                        {
                                            $sort_option = "DATE(STR_TO_DATE(tsm.created_date, '%d/%m/%Y, %H:%i:%s')) = CURDATE()";
                                        }

                                        elseif($_GET['date_range'] == "weekly")
                                        {
                                            $sort_option = "
                                                            WEEK(STR_TO_DATE(tsm.created_date, '%d/%m/%Y, %H:%i:%s')) = WEEK(CURDATE())
                                                            AND YEAR(STR_TO_DATE(tsm.created_date, '%d/%m/%Y, %H:%i:%s')) = YEAR(CURDATE())
                                                        ";
                                        }
                                        ...
                                        elseif($_GET['date_range'] == "monthly")
                                        {
                                            ...
                                            $sort_option = "
                                                            MONTH(STR_TO_DATE(tsm.created_date, '%d/%m/%Y, %H:%i:%s')) = MONTH(CURDATE())
                                                            AND YEAR(STR_TO_DATE(tsm.created_date, '%d/%m/%Y, %H:%i:%s')) = YEAR(CURDATE())
                                                        "; 
                                        ...
                                        {
                                            $sort_option = " 
                                                            MONTH(STR_TO_DATE(tsm.created_date, '%d/%m/%Y, %H:%i:%s')) = MONTH(DATE_SUB(CURDATE(), INTERVAL 1 MONTH))
                                                            AND YEAR(STR_TO_DATE(tsm.created_date, '%d/%m/%Y, %H:%i:%s')) = YEAR(DATE_SUB(CURDATE(), INTERVAL 1 MONTH))
                                                            ";  
                                        }

                                        elseif($_GET['date_range'] == "year")
                                        {
                                            $sort_option = "YEAR(STR_TO_DATE(tsm.created_date, '%d/%m/%Y, %H:%i:%s')) = YEAR(CURDATE())";
                                        }

                                        elseif($_GET['date_range'] == "lastyear")
                                        {
                                            $sort_option = "YEAR(STR_TO_DATE(tsm.created_date, '%d/%m/%Y, %H:%i:%s')) = YEAR(CURDATE()) - 1";
                                        }
                                    }
                                    ...
                                    $sort_option = "STR_TO_DATE(tsm.created_date, '%d/%m/%Y, %H:%i:%s') BETWEEN STR_TO_DATE('$format_dt1', '%d/%m/%Y') AND STR_TO_DATE('$format_dt2', '%d/%m/%Y')";
                                    // Lakukan sesuatu dengan $sort_option, misalnya memproses data dari database
                                    }
                                    $sql = "SELECT 
                                                tsm.id_tr_set_marwa, 
                                                tsm.qty, 
                                                STR_TO_DATE(tsm.created_date, '%d/%m/%Y, %H:%i:%s') AS created, 
                                                tpsm.nama_set_marwa, 
                                                tpsm.kode_set_marwa, 
                                                ket_in.ket_in,
                                                mr.nama_merk, 
                                            ...
                                            LEFT JOIN keterangan_in ket_in ON(tsm.id_ket_in = ket_in.id_ket_in) 
                                            LEFT JOIN $database2.user AS uc ON (tsm.id_user = uc.id_user)
                                            WHERE $sort_option
                                    ...
                                    ?>
                                        <tr>
                                            ...
                                            <td class="text-end text-nowrap"><?php echo $data['qty'] ?></td>
                                            <td class="text-center text-nowrap"><?php echo $data['ket_in'] ?></td>
                                            <td class="text-center text-nowrap">
                                                <?php echo $data['user_created'] ?><br>
                                                (<?php echo date('d/m/Y, H:i:s', strtotime($data['created'])) ?>)
                                            </td>
                                            <?php  
                                                if ($role == "Super Admin" || $role == "Manager Gudang" || $role == "Admin Gudang") { 
                                                    ?>
