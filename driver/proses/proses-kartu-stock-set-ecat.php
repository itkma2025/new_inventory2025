...
$key = "KM@2024?";
$created_by = decrypt($_SESSION['tiket_id'], $key_global);;
...
if (isset($_POST['simpan'])) {
    ...
    $id_ket_in = $data_ket_in['id_ket_in'];
        ...
        if ($token_csrf != "") {
            if ($date_now > $exp_token) {
                ...
                try {
                    ...
                    $simpan_data_kartu_stock = "INSERT INTO kartu_stock_set_ecat 
                    ...
                    $simpan_history_kartu_stock = "INSERT INTO kartu_stock_set_ecat_history
                ...
                if ($date_now > $exp_token) {
                    ...
                    try {
                        ...
                        $simpan_data_kartu_stock = "INSERT INTO kartu_stock_set_ecat 
                                                        ...
                                                        VALUES
                                                        ('$id_kartu_stock', '$id_produk_decrypt', '$status', '$id_ket_out', '$id_spk', '$qty_in', '$qty_out', '$keterangan', '$date_now', '$created_by')";
                        if (!$connect->query($simpan_data_kartu_stock)) {
                            throw new Exception("Kesalahan dalam eksekusi simpan data kartu stock: " . $connect->error);
                        }
    ...
    $updated_by = decrypt($_SESSION['tiket_id'], $key_global);;
    ...
    if ($token_csrf != "") {
        if ($date_now > $exp_token) {
            ...
            try {
                ...
                // Eksekusi query SQL pertama
                $edit_in = "UPDATE kartu_stock_set_ecat SET keterangan = '$keterangan', qty_in = '$qty' WHERE id_kartu_stock = '$id_kartu_stock_decrypt'";
    ...
    if ($token_csrf != "") {
        if ($date_now > $exp_token) {
            ...
            try {
                ...
                // Eksekusi query SQL pertama
                $edit_out = "UPDATE kartu_stock_set_ecat SET keterangan = '$keterangan', qty_out = '$qty' WHERE id_kartu_stock = '$id_kartu_stock_decrypt'";
    ...
    try {
        ...
        // Eksekusi query SQL pertama
        $hapus = $connect->query("DELETE FROM kartu_stock_set_ecat WHERE id_kartu_stock = '$id_kartu_stock_decrypt'");
        if (!$hapus) {
            throw new Exception("Kesalahan dalam eksekusi simpan data kartu stock: " . $connect->error);
        }
