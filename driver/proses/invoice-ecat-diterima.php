...
if (isset($_POST['diterima'])) {
    ...
    $tgl_post = htmlspecialchars($_POST['tgl']); // hasil: 17/04/2025
    // Ubah format tanggal
    $tgl = DateTime::createFromFormat('d/m/Y', $tgl_post)->format('Y-m-d');
    ...
    if ($diterima_oleh == 'Customer') {
        ...
        try {
            ...
            $cek_bukti = mysqli_query($connect_ecat, "SELECT id_inv_ecat FROM inv_bukti_terima WHERE id_inv_ecat = '$id_inv_decrypt'");
            if (mysqli_num_rows($cek_bukti) > 0) {
                // Jika data sudah ada, lakukan update
                $bukti_terima = mysqli_query($connect_ecat, "UPDATE inv_bukti_terima 
                                                        SET bukti_terima = '$newFileName', lokasi = '$location', approval = '0', created_date = '$datetime', created_by = '$id_user'
            ...
            $cek_penerima = mysqli_query($connect_ecat, "SELECT id_inv_ecat FROM inv_penerima WHERE id_inv_ecat = '$id_inv_decrypt'");
            if (mysqli_num_rows($cek_penerima) > 0) {
                // Jika data sudah ada, lakukan update
                $query_diterima = mysqli_query($connect_ecat, "UPDATE inv_penerima 
                                                        SET nama_penerima = '$nama_penerima', alamat = '$alamat', tgl_terima = '$tgl' 
                                                        WHERE id_inv_ecat = '$id_inv_decrypt'");
            } else {
                // Jika data belum ada, lakukan insert
                $query_diterima = mysqli_query($connect_ecat, "INSERT INTO inv_penerima (id_inv_penerima, id_inv_ecat, nama_penerima, alamat, tgl_terima)
                                                        VALUES ('$id_inv_penerima', '$id_inv_decrypt', '$nama_penerima', '$alamat', '$tgl')");
            }

            // Query untuk update status_transaksi di inv_ecat
            $query_update_inv = mysqli_query($connect_ecat, "UPDATE inv_ecat SET status_transaksi = 'Diterima' WHERE id_inv_ecat = '$id_inv_decrypt'");

            // Query untuk update jenis_penerima di status_kirim
            $query_update_status = mysqli_query($connect_ecat, "UPDATE status_kirim SET jenis_penerima = 'Customer', id_ekspedisi = '', no_resi = '', status_review = '0' WHERE id_inv_ecat = '$id_inv_decrypt'");
        ...
        try {
            // Cek apakah data sudah ada di inv_bukti_terima$connect_ecat
            $cek_bukti = mysqli_query($connect_ecat, "SELECT id_inv_ecat FROM inv_bukti_terima WHERE id_inv_ecat = '$id_inv_decrypt'");
            if (mysqli_num_rows($cek_bukti) > 0) {
                // Jika data sudah ada, lakukan update
                $bukti_terima = mysqli_query($connect_ecat, "UPDATE inv_bukti_terima 
            ...
            // Query untuk update status_transaksi di inv_ecat
            $query_update_inv = mysqli_query($connect_ecat, "UPDATE inv_ecat SET ongkir = '$ongkir', status_transaksi = 'Dikirim' WHERE id_inv_ecat = '$id_inv_decrypt'");
