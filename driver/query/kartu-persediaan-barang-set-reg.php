<?php  
    ...
    $sql_produk_reg = "SELECT 
                            pr.id_set_marwa as produk_id,  
                            pr.nama_set_marwa as nama_produk,
                            pr.gambar,
                            pr.deskripsi,  
                            mr.nama_merk,
                            COALESCE(kp.nama_kategori, '-') as kat_prod,
                            COALESCE(kp.no_izin_edar, '-') as no_izin_edar,
                            lok.nama_lokasi,
                            lok.no_lantai,
                            lok.nama_area,
                            lok.no_rak,
                        ...
                        FROM tb_produk_set_marwa as pr
                        ...
                        LEFT JOIN stock_produk_reguler spr ON (pr.id_set_marwa = spr.id_produk_reg)
                        LEFT JOIN kartu_stock_set_reg ksr ON (pr.id_set_marwa = ksr.id_produk)
                        ...
                        LEFT JOIN $database2.user us ON (ksr.created_by = us.id_user)
                        WHERE pr.id_set_marwa ='$id_produk'";
    ...
    if ($get_sort == "semua_data"){
        $sort_data = "AND YEAR(ksr.created_date) = YEAR(CURDATE()) ORDER BY ksr.created_date ASC";
    ...
    $sql_total_stock = $sql_produk_reg . " AND YEAR(ksr.created_date) = YEAR(CURDATE()) ORDER BY ksr.created_date ASC";
