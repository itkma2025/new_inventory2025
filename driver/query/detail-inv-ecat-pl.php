<?php
    ...
    $sql_spk_trim = "SELECT DISTINCT
                    ...
                    FROM inv_pl AS pl
                    LEFT JOIN tb_spk_pl spk ON pl.id_inv_pl = spk.id_inv_pl
                    ...
                    WHERE pl.id_inv_pl = '$id_inv_decrypt' ";
    ...
    $sql_produk = " SELECT DISTINCT
                        ...
                        tpr.harga_produk
                    FROM 
                        transaksi_produk_pl AS tps
                    LEFT JOIN 
                        tb_spk_pl AS sr ON sr.id_spk_pl = tps.id_spk
                    LEFT JOIN 
                        $db.stock_produk_ecat AS spr ON tps.id_produk = spr.id_produk_ecat
                    LEFT JOIN 
                        $db.tb_produk_set_ecat AS tpsm ON (id_produk_ecat = spr.id_produk_ecat)
                    LEFT JOIN 
                    ...
                    WHERE 
                        tps.status_trx = 1 AND sr.id_inv_pl = '$id_inv_decrypt'
                    ORDER BY
                        sr.created_date ASC";

                    
    // Data Untuk Menampilkan detail
    $spk_trim =  $connect_ecat->query($sql_spk_trim . " GROUP BY spk.no_spk_pl ORDER BY spk.no_spk_pl ASC");
